...
def main():
    ...
    print(
        f"Shannon diversity - Mean: {shannon_div.mean():.3f} ± {shannon_div.std():.3f}"
    )
    print(
        f"Simpson diversity - Mean: {simpson_div.mean():.3f} ± {simpson_div.std():.3f}"
    )
    print(f"Species richness - Mean: {richness.mean():.1f} ± {richness.std():.1f}")
    print(
        f"Fisher's alpha - Mean: {fisher_alpha.mean():.3f} ± {fisher_alpha.std():.3f}"
    )
    ...
    axes[0, 1].hist(
        simpson_div, bins=10, alpha=0.7, color="lightcoral", edgecolor="black"
    )
    ...
    print(
        f"Euclidean distance - Mean: {np.mean(euclidean_dist[euclidean_dist > 0]):.3f}"
    )
